...
const LoginNavbar = ({ minimal = false }) => {
  ...
  useEffect(() => {
    ...
    if (email) {
      fetch(`http://localhost:8081/api/project/volunteers/email/${email}`)
        .then((res) => res.json())
        .then((data) => {
          ...
          <div className="d-none d-md-flex gap-2 position-relative" ref={dropdownRef}>
            ...
            {dropdownOpen && (
              <ul className="dropdown-menu dropdown-menu-end show mt-2" style={{ display: "block", position: "absolute", right: 0 }}>
                <li><Link to="/volundashboard" className="dropdown-item">Profile</Link></li>
      ...
      {!minimal && mobileMenuOpen && (
        <div className="d-md-none mt-3 border-top pt-3">
          <div className="mt-3 d-flex flex-column gap-2">
            <Link to="/volundashboard" className="text-decoration-none text-dark" onClick={() => setMobileMenuOpen(false)}>
